...
SET BITS=64
SET USER_JDK64_FILE=%APPDATA%\JetBrains\PyCharm2020.1\pycharm%BITS%.exe.jdk
SET BITS=
SET USER_JDK_FILE=%APPDATA%\JetBrains\PyCharm2020.1\pycharm%BITS%.exe.jdk
...
IF NOT EXIST "%VM_OPTIONS_FILE%" (
  :: user-overridden
  SET VM_OPTIONS_FILE=%APPDATA%\JetBrains\PyCharm2020.1\pycharm%BITS%.exe.vmoptions
)
IF NOT EXIST "%VM_OPTIONS_FILE%" (
...
SET COMMON_JVM_ARGS="-XX:ErrorFile=%USERPROFILE%\java_error_in_PYCHARM_%%p.log" "-XX:HeapDumpPath=%USERPROFILE%\java_error_in_PYCHARM.hprof" -Didea.paths.selector=PyCharm2020.1 %IDE_PROPERTIES_PROPERTY%
SET IDE_JVM_ARGS=-Didea.platform.prefix=Python
